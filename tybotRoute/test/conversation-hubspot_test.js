...
describe('Conversation for hubspot test', async () => {
  ...
  it('/hubspot failure - return code 409', (done) => {
    ...
    endpointServer.post('/:projectId/requests/:requestId/messages', function (req, res) {
      ...
      const command2 = message.attributes.commands[2];
      assert(command2.type === "message");
      assert(command2.message.text === 'Hubspot error is: Contact already exists. Existing ID: 801');
  ...
  it('/hubspot failure - return code 401', (done) => {
    ...
    endpointServer.post('/:projectId/requests/:requestId/messages', function (req, res) {
      ...
      const command2 = message.attributes.commands[2];
      assert(command2.type === "message");
      assert(command2.message.text === 'Hubspot error is: Authentication credentials not found. This API supports OAuth 2.0 authentication and you can find more details at https://developers.hubspot.com/docs/methods/auth/oauth-overview');
  ...
  it('/hubspot failure - return code 400', (done) => {
    ...
    endpointServer.post('/:projectId/requests/:requestId/messages', function (req, res) {
      ...
      const command2 = message.attributes.commands[2];
      assert(command2.type === "message");
      assert(command2.message.text === 'Hubspot error is: Property values were not valid: [{\"isValid\":false,\"message\":\"Email address dsfsafas@ is invalid\",\"error\":\"INVALID_EMAIL\",\"name\":\"email\"}]');
