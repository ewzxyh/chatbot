...
const PROJECT_ID = process.env.TEST_ACTIONS_PROJECT_ID;
const REQUEST_ID = "support-group-" + PROJECT_ID + "-" + uuidv4().replace(/-/g, "");
const BOT_ID = process.env.TEST_ACTIONS_BOT_ID;
const CHATBOT_TOKEN = process.env.ACTIONS_CHATBOT_TOKEN;

console.log("ACTIONS Testing conversation setup:");
console.log("ACTIONS PROJECT_ID:", PROJECT_ID);
console.log("ACTIONS REQUEST_ID:", REQUEST_ID);
console.log("ACTIONS BOT_ID:", BOT_ID);
...
describe('Conversation1', async() => {
    ...
    it('/all_filled (none) => /form_to_unfill => (fill) => /all_filled (all) /form_to_unfill (bypass because filled) => /delete_fullname => all_filled (no fullname) => /form_to_unfill (verify it asks only for fullname) => all_filled (all, again)', (done) => {
      ...
      endpointServer.post('/:projectId/requests/:requestId/messages', function (req, res) {
        ...
        else if (message.text === "Thanks Andrea\nYour email redacted@example.invalid" && message.triggeredByMessageId === request3_uuid) {
        ...
        else if (message.text === "You filled\nfullname: Andrea\nyouremail: redacted@example.invalid" && message.triggeredByMessageId === request4_uuid) {
        ...
        else if (message.text === "Thanks Andrea\nYour email redacted@example.invalid" && message.triggeredByMessageId === request5_uuid) {
        ...
        else if (message.text === "Thanks John\nYour email redacted@example.invalid" && message.triggeredByMessageId === request8_uuid) {
